...
router.patch('/:name', function(req, res){
    ...
    User.findOne({userName})
        .then((user) => {
            ...
            //find recipe index by name,
            const recipeIndex = user.recipe.findIndex((r) => r.name === recipeName); //r can be considered as a variable iteratin over recipie in recipie-list
            if (!recipe){
                res.status(404).json({ message: 'Recipe not found' });
            }
            //update recipe
            user.recipe[recipeIndex] = {
                name: req.body.name,
                ingredients: req.body.ingredients,
                description: req.body.description,
            };
